...
echo "=== 运行单元测试（单线程模式）==="
uv run pytest tests/ -v -n0 --ignore=tests/integration/ --ignore=tests/manual_test_hook_interaction_scenarios.py 2>&1 | tee "$TEMP_FILE"
...
uv run pytest tests/integration/ -v -n0 2>&1 | tee "$TEMP_FILE"
